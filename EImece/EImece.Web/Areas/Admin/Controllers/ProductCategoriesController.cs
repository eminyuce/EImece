...
public sealed class ProductCategoriesController : BaseAdminController
{
    private readonly EImeceDbContext _db;

    public ProductCategoriesController(IOptions<EImeceOptions> siteOptions, EImeceDbContext db) : base(siteOptions)
    {
        _db = db;
    }
    ...
    public async Task<IActionResult> Index(string? search, int page = 1, int pageSize = 25, string? sort = null, string? sortDir = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            query = (grid.Sort?.ToLowerInvariant()) switch
            {
                ...
                "position" => grid.SortDir == "asc" ? query.OrderBy(p => p.Position) : query.OrderByDescending(p => p.Position),
                _ => query.OrderByDescending(p => p.Id)
            };

            var total = await query.CountAsync(cancellationToken).ConfigureAwait(false);
            var items = await query.ApplyPaging(grid)
                .Select(p => new { p.Id, p.Name, p.IsActive, p.Position })
                .ToListAsync(cancellationToken).ConfigureAwait(false);

            var rows = items.Select(x => (IReadOnlyList<string?>)new string?[] { x.Id.ToString(), x.Name, x.IsActive ? "Evet" : "Hayır", x.Position.ToString() });
            return EntityList(BuildList("Ürün Kategorileri", "ProductCategories", new[] { "Id", "Name", "IsActive", "Position" }, rows, grid.Search,
                totalCount: total, grid: grid, ajaxDeleteAction: "DeleteProductCategoriesGridItem"));
        }
        catch (Exception ex)
        {
            return EntityList(BuildList("Ürün Kategorileri", "ProductCategories", new[] { "Id", "Name", "IsActive", "Position" }, Array.Empty<IReadOnlyList<string?>>(), grid.Search, ex.Message, grid: grid));
        }
    }
    ...
    public async Task<IActionResult> SaveOrEdit(int id = 0, CancellationToken cancellationToken = default)
    {
        if (id <= 0)
        {
            ...
            {
                Title = "Ürün Kategorileri — Yeni",
                ControllerName = "ProductCategories",
                EditProfile = "productcategory",
                Id = 0,
                Name = string.Empty,
                IsActive = true
            });
        ...
        var entity = await _db.ProductCategories.AsNoTracking().FirstOrDefaultAsync(p => p.Id == id, cancellationToken).ConfigureAwait(false);
        if (entity is null) return NotFound();

        return View("~/Areas/Admin/Views/Shared/EntityEdit.cshtml", new AdminEditViewModel
        {
            Title = "Ürün Kategorileri — Düzenle",
            ControllerName = "ProductCategories",
            EditProfile = "productcategory",
            Id = entity.Id,
    ...
    {
        model.EditProfile = "productcategory";
        if (string.IsNullOrWhiteSpace(model.Name))
        {
            ModelState.AddModelError(nameof(model.Name), "Name is required");
            model.Title = "Ürün Kategorileri";
            return View("~/Areas/Admin/Views/Shared/EntityEdit.cshtml", model);
        }

        if (model.Id > 0)
        {
            var entity = await _db.ProductCategories.FirstOrDefaultAsync(p => p.Id == model.Id, cancellationToken).ConfigureAwait(false);
            if (entity is null) return NotFound();
        ...
        {
            var entityType = _db.ProductCategories.EntityType.ClrType;
    ...
    {
        var entity = await _db.ProductCategories.FirstOrDefaultAsync(p => p.Id == id, cancellationToken).ConfigureAwait(false);
        if (entity is null) return NotFound();
