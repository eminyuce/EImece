...
public sealed class MainPageImagesController : BaseAdminController
{
    private readonly EImeceDbContext _db;

    public MainPageImagesController(IOptions<EImeceOptions> siteOptions, EImeceDbContext db) : base(siteOptions)
    {
        _db = db;
    }
    ...
    public async Task<IActionResult> Index(string? search, int page = 1, int pageSize = 25, string? sort = null, string? sortDir = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var query = _db.MainPageImages.AsNoTracking()
            ...
            return EntityList(BuildList("Ana Sayfa Görselleri", "MainPageImages", new[] { "Id", "Name", "IsActive" }, rows, grid.Search,
                totalCount: total, grid: grid));
        }
        catch (Exception ex)
        {
            return EntityList(BuildList("Ana Sayfa Görselleri", "MainPageImages", new[] { "Id", "Name", "IsActive" }, Array.Empty<IReadOnlyList<string?>>(), grid.Search, ex.Message, grid: grid));
        }
    }
    ...
    public async Task<IActionResult> SaveOrEdit(int id = 0, CancellationToken cancellationToken = default)
    {
        if (id <= 0)
        {
            ...
            {
                Title = "Ana Sayfa Görselleri — Yeni",
        ...
        var entity = await _db.MainPageImages.AsNoTracking().FirstOrDefaultAsync(p => p.Id == id, cancellationToken).ConfigureAwait(false);
        if (entity is null) return NotFound();

        return View("~/Areas/Admin/Views/Shared/EntityEdit.cshtml", new AdminEditViewModel
        {
            Title = "Ana Sayfa Görselleri — Düzenle",
            ControllerName = "MainPageImages",
            Id = entity.Id,
    ...
    public async Task<IActionResult> SaveOrEdit(AdminEditViewModel model, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(model.Name))
        {
            ...
            model.Title = "Ana Sayfa Görselleri";
            return View("~/Areas/Admin/Views/Shared/EntityEdit.cshtml", model);
        }

        if (model.Id > 0)
        {
            var entity = await _db.MainPageImages.FirstOrDefaultAsync(p => p.Id == model.Id, cancellationToken).ConfigureAwait(false);
            if (entity is null) return NotFound();
        ...
        {
            var entityType = _db.MainPageImages.EntityType.ClrType;
    ...
    {
        var entity = await _db.MainPageImages.FirstOrDefaultAsync(p => p.Id == id, cancellationToken).ConfigureAwait(false);
        if (entity is null) return NotFound();
        _db.MainPageImages.Remove(entity);
        await _db.SaveChangesAsync(cancellationToken).ConfigureAwait(false);
