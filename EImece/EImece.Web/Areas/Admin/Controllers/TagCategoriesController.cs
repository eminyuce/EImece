...
public sealed class TagCategoriesController : BaseAdminController
{
    private readonly EImeceDbContext _db;

    public TagCategoriesController(IOptions<EImeceOptions> siteOptions, EImeceDbContext db) : base(siteOptions)
    {
        _db = db;
    }
    ...
    public async Task<IActionResult> Index(string? search, int page = 1, int pageSize = 25, string? sort = null, string? sortDir = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var query = _db.TagCategories.AsNoTracking()
            ...
            return EntityList(BuildList("Etiket Kategorileri", "TagCategories", new[] { "Id", "Name", "IsActive" }, rows, grid.Search,
                totalCount: total, grid: grid));
        }
        catch (Exception ex)
        {
            return EntityList(BuildList("Etiket Kategorileri", "TagCategories", new[] { "Id", "Name", "IsActive" }, Array.Empty<IReadOnlyList<string?>>(), grid.Search, ex.Message, grid: grid));
        }
    }
    ...
    public async Task<IActionResult> SaveOrEdit(int id = 0, CancellationToken cancellationToken = default)
    {
        if (id <= 0)
        {
            ...
            {
                Title = "Etiket Kategorileri — Yeni",
                ControllerName = "TagCategories",
                Id = 0,
                Name = string.Empty,
                IsActive = true
            });
        ...
        var entity = await _db.TagCategories.AsNoTracking().FirstOrDefaultAsync(p => p.Id == id, cancellationToken).ConfigureAwait(false);
        if (entity is null) return NotFound();

        return View("~/Areas/Admin/Views/Shared/EntityEdit.cshtml", new AdminEditViewModel
        {
            Title = "Etiket Kategorileri — Düzenle",
            ControllerName = "TagCategories",
            Id = entity.Id,
    ...
    public async Task<IActionResult> SaveOrEdit(AdminEditViewModel model, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(model.Name))
        {
            ...
            model.Title = "Etiket Kategorileri";
            return View("~/Areas/Admin/Views/Shared/EntityEdit.cshtml", model);
        }

        if (model.Id > 0)
        {
            var entity = await _db.TagCategories.FirstOrDefaultAsync(p => p.Id == model.Id, cancellationToken).ConfigureAwait(false);
            if (entity is null) return NotFound();
        ...
        {
            var entityType = _db.TagCategories.EntityType.ClrType;
    ...
    {
        var entity = await _db.TagCategories.FirstOrDefaultAsync(p => p.Id == id, cancellationToken).ConfigureAwait(false);
        if (entity is null) return NotFound();
        _db.TagCategories.Remove(entity);
        await _db.SaveChangesAsync(cancellationToken).ConfigureAwait(false);
