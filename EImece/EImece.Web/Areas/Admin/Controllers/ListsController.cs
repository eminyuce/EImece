...
public sealed class ListsController : BaseAdminController
{
    private readonly EImeceDbContext _db;

    public ListsController(IOptions<EImeceOptions> siteOptions, EImeceDbContext db) : base(siteOptions)
    {
        _db = db;
    }
    ...
    public async Task<IActionResult> Index(string? search, int page = 1, int pageSize = 25, string? sort = null, string? sortDir = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var query = _db.Lists.AsNoTracking()
            ...
            return EntityList(BuildList("Listeler", "Lists", new[] { "Id", "Name", "IsActive" }, rows, grid.Search,
                totalCount: total, grid: grid));
        }
        catch (Exception ex)
        {
            return EntityList(BuildList("Listeler", "Lists", new[] { "Id", "Name", "IsActive" }, Array.Empty<IReadOnlyList<string?>>(), grid.Search, ex.Message, grid: grid));
        }
    }
    ...
    public async Task<IActionResult> SaveOrEdit(int id = 0, CancellationToken cancellationToken = default)
    {
        ...
        var entity = await _db.Lists.AsNoTracking().FirstOrDefaultAsync(p => p.Id == id, cancellationToken).ConfigureAwait(false);
        if (entity is null) return NotFound();

        return View("~/Areas/Admin/Views/Shared/EntityEdit.cshtml", new AdminEditViewModel
        {
            Title = "Listeler — Düzenle",
            ControllerName = "Lists",
            Id = entity.Id,
    ...
    public async Task<IActionResult> SaveOrEdit(AdminEditViewModel model, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(model.Name))
        {
            ...
            model.Title = "Listeler";
            return View("~/Areas/Admin/Views/Shared/EntityEdit.cshtml", model);
        }

        if (model.Id > 0)
        {
            var entity = await _db.Lists.FirstOrDefaultAsync(p => p.Id == model.Id, cancellationToken).ConfigureAwait(false);
            if (entity is null) return NotFound();
        ...
        {
            var entityType = _db.Lists.EntityType.ClrType;
    ...
    {
        var entity = await _db.Lists.FirstOrDefaultAsync(p => p.Id == id, cancellationToken).ConfigureAwait(false);
        if (entity is null) return NotFound();
