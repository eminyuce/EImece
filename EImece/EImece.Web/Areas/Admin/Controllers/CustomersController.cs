...
public sealed class CustomersController : BaseAdminController
{
    private readonly EImeceDbContext _db;

    public CustomersController(IOptions<EImeceOptions> siteOptions, EImeceDbContext db) : base(siteOptions)
    {
        _db = db;
    }
        ...
        try
        {
            var query = _db.Customers.AsNoTracking()
                .Where(p => string.IsNullOrWhiteSpace(grid.Search)
                    ...
                    || (p.Email != null && p.Email.Contains(grid.Search)));

            query = (grid.Sort?.ToLowerInvariant()) switch
            {
                "name" => grid.SortDir == "asc" ? query.OrderBy(p => p.Name) : query.OrderByDescending(p => p.Name),
                "email" => grid.SortDir == "asc" ? query.OrderBy(p => p.Email) : query.OrderByDescending(p => p.Email),
                _ => query.OrderByDescending(p => p.Id)
            };

            var total = await query.CountAsync(cancellationToken).ConfigureAwait(false);
            var items = await query.ApplyPaging(grid)
                .Select(p => new { p.Id, p.Name, p.Email, p.GsmNumber })
                .ToListAsync(cancellationToken).ConfigureAwait(false);

            var rows = items.Select(x => (IReadOnlyList<string?>)new string?[] { x.Id.ToString(), x.Name, x.Email, x.GsmNumber });
            return EntityList(BuildList("Müşteriler", "Customers", new[] { "Id", "Name", "Email", "GsmNumber" }, rows, grid.Search,
                totalCount: total, grid: grid));
        }
        catch (Exception ex)
        {
            return EntityList(BuildList("Müşteriler", "Customers", new[] { "Id", "Name", "Email", "GsmNumber" }, Array.Empty<IReadOnlyList<string?>>(), grid.Search, ex.Message, grid: grid));
        }
    }
    ...
    public async Task<IActionResult> SaveOrEdit(int id = 0, CancellationToken cancellationToken = default)
    {
        ...
        var entity = await _db.Customers.AsNoTracking().FirstOrDefaultAsync(p => p.Id == id, cancellationToken).ConfigureAwait(false);
        if (entity is null) return NotFound();

        return View("~/Areas/Admin/Views/Shared/EntityEdit.cshtml", new AdminEditViewModel
        {
            Title = "Müşteriler — Düzenle",
            ControllerName = "Customers",
            Id = entity.Id,
    ...
    public async Task<IActionResult> SaveOrEdit(AdminEditViewModel model, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(model.Name))
        {
            ...
            model.Title = "Müşteriler";
            return View("~/Areas/Admin/Views/Shared/EntityEdit.cshtml", model);
        }

        if (model.Id > 0)
        {
            var entity = await _db.Customers.FirstOrDefaultAsync(p => p.Id == model.Id, cancellationToken).ConfigureAwait(false);
            if (entity is null) return NotFound();
        ...
        {
            var entityType = _db.Customers.EntityType.ClrType;
    ...
    {
        var entity = await _db.Customers.FirstOrDefaultAsync(p => p.Id == id, cancellationToken).ConfigureAwait(false);
        if (entity is null) return NotFound();
