...
| 16 | **Third-party design assets introduce vulnerable JS** | Existing views already load CDN Bootstrap 3/4/5, jQuery, CKEditor, AddThis | Medium | Vendor design assets into `Content/designs/{d}/`; no new CDN origins; note there is **no CSP** today (`SecurityHeadersHttpModule.cs`), so a CSP is a worthwhile separate hardening task |
