    ...
    public class DashboardController : BaseAdminController
    {
        ...
        private void ExecuteWarmUpSql()
        {
            try
            {
                ...
                string.Format("%s%s%s%s", myUri.Scheme, myUri.Host, myUri.Port, myUri.GetLeftPart(UriPartial.Authority));


                var buffer = GeneralHelper.GetImageFromUrl(Request.Url + "/sitemap.xml");
