...
function getProductSpecItem() {
    return new ProductSpecItem();
}
$("#AddToCart").click(function () {
    ...
    $('[data-product-selected-specs=' + nProductId + ']').each(function () {
        var pp = getProductSpecItem();
        pp.specsName = $(this).attr('name');
        pp.specsValue = $(this).val();
        selectedTotalSpecs.push(pp);
    });

    var postData333 = JSON.stringify({ selectedTotalSpecs });
    console.log(postData333);

    var postData = JSON.stringify({
        productId: nProductId,
        quantity: $("#quantity").val(),
        orderGuid: getOrderGuid(),
        selectedTotalSpecs: postData333
    });
    console.log(postData);
    ajaxMethodCall(postData, "/Payment/AddToCart", function (data) {
        GetShoppingCartLinks();
