    ...
    public class PaymentController : BaseController
    {
        ...
        public ActionResult PaymentResult(RetrieveCheckoutFormRequest model, string o, string u)
        {
            ...
            if (checkoutForm.PaymentStatus.Equals(Domain.Constants.SUCCESS, StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                Task.Run(() =>
                {
                    OrderConfirmationEmailRazorTemplate pp = MailTemplateService.GenerateOrderConfirmationEmailRazorTemplate(order.Id);
                    EmailSender.SendOrderConfirmationEmail(SettingService.GetEmailAccount(), shoppingCart, pp);
                });
