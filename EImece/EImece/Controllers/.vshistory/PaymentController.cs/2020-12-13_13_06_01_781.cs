...
namespace EImece.Controllers
{
    public class PaymentController : BaseController
    {
        ...
        public ActionResult AddToCart(int productId, int quantity, string orderGuid, string selectedTotalSpecs)
        {
            ...
            if (item.ProductSpecItems.IsEmpty())
            {
                item.ProductSpecItems = new List<ProductSpecItem>();
            }

            if (!string.IsNullOrEmpty(selectedTotalSpecs))
            {
                var ooo = JsonConvert.DeserializeObject<List<ProductSpecItem>>(selectedTotalSpecs);
                item.ProductSpecItems.AddRange(ooo);
            }
            ...
            PaymentLogger.Info(JsonConvert.SerializeObject(shoppingCart)+"  "+ selectedTotalSpecs);
