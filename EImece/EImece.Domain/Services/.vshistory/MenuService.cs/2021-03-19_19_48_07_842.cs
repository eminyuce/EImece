...
namespace EImece.Domain.Services
{
    public class MenuService : BaseContentService<Menu>, IMenuService
    {
        ...
        public MenuPageViewModel GetPageByMenuLink(string menuLink, int? language)
        {
            ...
            int lang = language.HasValue ? 0 : language.Value;
            var menu = lists.FirstOrDefault(r => r.Lang == language.Value && r.MenuLink.Equals(menuLink, StringComparison.InvariantCultureIgnoreCase));
            if (menu == null)
            {
                return null;
            }
