...
namespace EImece.Domain.Helpers
{
    ...
    public static class StringConverter
    {
        ...
        private static readonly Dictionary<Type, Dictionary<Type, Type>> TypeMap = InitializeTypeMap();
        ...
        public static Type FindCommonType(Type typeA, Type typeB)
        {
            ...
            Dictionary<Type, Type> innerMap;
            if (!TypeMap.TryGetValue(typeA, out innerMap))
                return typeof(string);

            Type commonType;
            if (!innerMap.TryGetValue(typeB, out commonType))
                return typeof(string);

            return commonType;
        }

        private static Dictionary<Type, Dictionary<Type, Type>> InitializeTypeMap()
        {
            return new Dictionary<Type, Dictionary<Type, Type>>()
            {
                // Comparing byte
