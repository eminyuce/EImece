...
namespace EImece.Domain.Repositories
{
       ...
       where T : class, IEntity<int>
    {
        ...
        protected BaseRepository(IEImeceContext dbContext, ILogger logger) : base(dbContext)
        {
            DbContext = dbContext;
            Logger = logger ?? throw new ArgumentNullException(nameof(logger));
        }

        protected BaseRepository(IEImeceContext dbContext) : this(dbContext, NullLogger.Instance)
        {
        }
        ...
        public virtual bool DeleteByWhereCondition(Expression<Func<T, bool>> whereLambda)
        {
            ...
            using (var transactionResult = this.GetDbContext().Database.BeginTransaction(System.Data.IsolationLevel.ReadCommitted))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Logger.LogError(ex, "DeleteEntityByWhere");
                    throw;
                }
            }
            return isResult;
        ...
        public virtual int SaveOrEdit(T item)
        {
            ...
            {
                Logger.LogError(ex, BuildEntityValidationErrorMessage(ex));
                throw;
            }
        }
